...
const Post = ({ post, setCurrentId }) => {
  ...
  const userId = user?.result?.sub || user?.result?._id; // ✅ works for Google & custom auth
  ...
  const Likes = () => {
    if (likes.length > 0) {
      return hasLikedPost
        ? (
          <><ThumbUpAltIcon fontSize="small" />&nbsp;{likes.length > 1 ? `You and ${likes.length - 1} others` : 'You liked this'}</>
        )
        : (
